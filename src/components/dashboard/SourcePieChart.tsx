...
import { BRAND_CHARCOAL, BRAND_MIDNIGHT_BLUE, CHART_ADS, CHART_WEBSITE } from "@/lib/brand-colors";

interface SourcePieChartProps {
  data: Lead[];
}

const COLORS = [CHART_ADS, CHART_WEBSITE];
        ...
        <ResponsiveContainer width="100%" height={260}>
          <PieChart>
            <Pie
              ...
              nameKey="name"
              strokeWidth={0}
              label={({ value, cx, cy, midAngle, innerRadius: ir, outerRadius: or, payload }) => {
                const RADIAN = Math.PI / 180;
                const radius = (ir + or) / 2;
                const x = cx + radius * Math.cos(-midAngle * RADIAN);
                const y = cy + radius * Math.sin(-midAngle * RADIAN);
                const websiteSlice = String(payload?.name || "").toLowerCase().includes("website");
                return (
                  <text
                    x={x} y={y} textAnchor="middle" dominantBaseline="central"
                    fill={websiteSlice ? BRAND_CHARCOAL : BRAND_MIDNIGHT_BLUE}
