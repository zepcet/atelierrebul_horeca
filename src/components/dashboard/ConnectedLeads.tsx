...
const tierColor: Record<number, string> = {
  1: "bg-green-500/10 text-green-700 border-green-500/30",
  2: "bg-yellow-500/10 text-yellow-700 border-yellow-500/30",
  3: "bg-blue-500/10 text-blue-700 border-blue-500/30",
        ...
        return (
          <div key={lead.id} className="border border-border rounded-lg bg-card p-4 hover:shadow-md transition-shadow space-y-3">
            ...
            <div className="flex items-center gap-3">
              {lead.ig_username && (
                <a href={`https://instagram.com/${lead.ig_username}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">
                  <Instagram className="h-3 w-3" />@{lead.ig_username}
                </a>
              )}
              {lead.website && (
                <a href={lead.website.startsWith("http") ? lead.website : `https://${lead.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border">
              <Badge className="text-[10px] bg-green-500/10 text-green-700 border-green-500/30" variant="outline">Connected</Badge>
