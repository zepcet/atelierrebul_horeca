...
test.describe("Dashboard (requires auth)", () => {
  ...
  test("All Leads page renders", async ({ page }) => {
    await expect(page.getByText("All Leads")).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("Total Leads")).toBeVisible();
  });

  test("can navigate to Ads Leads", async ({ page }) => {
    await page.getByRole("link", { name: /Ads Leads/i }).click();
    await expect(page.getByText("Ads Leads")).toBeVisible({ timeout: 10000 });
  });

  test("can navigate to Website Leads", async ({ page }) => {
    await page.getByRole("link", { name: /Website Leads/i }).click();
    await expect(page.getByText("Website Leads")).toBeVisible({ timeout: 10000 });
  });
